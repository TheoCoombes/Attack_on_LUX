...
for episode in range(total_num_episodes):
    ...
    if episode % 50 == 0:
        print("AVERAGE REWARD", sum(model.rewards[-10:]) / 10, "AVERAGE MOVES", sum(cumulative_moves) / len(cumulative_moves))
        pygame.display.set_caption(f"AVERAGE REWARD {sum(model.rewards[-10:]) / 10} AVERAGE MOVES {sum(cumulative_moves) / len(cumulative_moves)}")
        cumulative_moves = []
